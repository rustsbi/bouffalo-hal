...
mod tests {
    ...
    fn struct_control_functions() {
        ...
        control = Control(0);
        assert_eq!(control.t_endian(), Endian::Little);
        control.set_t_endian(Endian::Big);
        assert_eq!(control.t_endian(), Endian::Big);
        assert_eq!(control.0, 0x1000);

        // Test h_endian functions
        control = Control(0);
        assert_eq!(control.h_endian(), Endian::Little);
        control.set_h_endian(Endian::Big);
        assert_eq!(control.h_endian(), Endian::Big);
        assert_eq!(control.0, 0x2000);

        // Test x_endian functions
        control = Control(0);
        assert_eq!(control.x_endian(), Endian::Little);
        control.set_x_endian(Endian::Big);
        assert_eq!(control.x_endian(), Endian::Big);
