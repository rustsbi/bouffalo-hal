...
```bash
rust-objcopy --binary-architecture=riscv64 --strip-all -O binary ./target/riscv64imac-unknown-none-elf/release/i2c-demo ./target/riscv64imac-unknown-none-elf/release/i2c-demo.bin
```
...
```bash
cargo run --target riscv64imac-unknown-none-elf --release -p i2c-demo -- --port PORT_NAME
```
