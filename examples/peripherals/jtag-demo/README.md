...
```bash
rust-objcopy --binary-architecture=riscv64 --strip-all -O binary ./target/riscv64imac-unknown-none-elf/release/jtag-demo ./target/riscv64imac-unknown-none-elf/release/jtag-demo.bin
```
...
```bash
cargo run --target riscv64imac-unknown-none-elf --release -p jtag-demo -- --port PORT_NAME
```
