...
```bash
rust-objcopy --binary-architecture=riscv64 --strip-all -O binary ./target/riscv64imac-unknown-none-elf/release/lz4d-demo ./target/riscv64imac-unknown-none-elf/release/lz4d-demo.bin
```
...
```bash
cargo run --target riscv64imac-unknown-none-elf --release -p lz4d-demo -- --port PORT_NAME
```
